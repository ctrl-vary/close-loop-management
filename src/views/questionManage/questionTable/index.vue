<template>
  ...
  <div class="app-container">
    <el-form ref="queryForm" size="small" :inline="true" label-width="68px">
      ...
      <el-form-item>
        <el-button
          ...
          >搜索</el-button
        >
        <el-button icon="el-icon-refresh" size="mini" @click="btnResetSearch"
          >重置</el-button
        >
      </el-form-item>
    </el-form>
          ...
          <template slot-scope="scope">
            <div v-if="hanleTableCol == 'close'">
              <el-button
                ...
                type="text"
                icon="el-icon-check"
                @click="btnCheckProcess(scope.row.quesId)"
                >流程追踪
              </el-button>
            </div>
            <div v-else-if="hanleTableCol == 'handle'">
              <el-button
                ...
                type="text"
                icon="el-icon-check"
                @click="btnCheckEvidence(scope.row.quesId)"
                >证据查看
              </el-button>
              <el-button
                size="mini"
                type="text"
                icon="el-icon-top"
                @click="btnUploadEvidence(scope.row)"
                >整改提交
              </el-button>
              <el-button
                size="mini"
                type="text"
                icon="el-icon-bottom"
                @click="btnReleaseQues(scope.row.quesId)"
                >继续下发
              </el-button>
            </div>
            <div v-else>
              <el-button
                ...
                type="text"
                icon="el-icon-check"
                @click="btnOnlyCheckEvidence(scope.row.quesId)"
              >
                整改查看</el-button
              >
            </div>
    ...
    <el-dialog title="证据上传" :visible.sync="upLoadDialogVisible" width="35%">
      ...
      >
        ...
        <div slot="tip" class="el-upload__tip">
          只能上传"png", "jpg", "jpeg"文件
        </div>
      </el-upload>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="upLoadDialogVisible = false"
          >取 消</el-button
        >
        <el-button size="mini" type="primary" @click="btnConfirmUpload"
          >确定上传</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <el-form>
        <el-form-item label="证据查看:">
          <span>{{ tempEvidence.description }} </span>
        </el-form-item>
        <el-form-item
          v-for="(src, index) in tempEvidence.picurl"
          :key="index + src"
          label="照片:"
        >
          <img style="width: 70%; height: 120px" :src="src" alt="" />
        </el-form-item>
      </el-form>

      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="checkEvidenceVisible = false"
          >取 消</el-button
        >
        <el-button size="mini" type="primary" @click="btnChangeEvidence"
          >更改证据</el-button
        >
      </span>
    </el-dialog>
    ...
    <el-dialog
      title="证据查看"
      :visible.sync="checkOnlyEvidenceVisible"
      width="30%"
    >
      <el-form label-width="90px">
        ...
        <el-form-item label="审核备注:">
          <el-input
            type="textarea"
            :rows="2"
            v-model="checkParams.remarks"
          ></el-input>
        </el-form-item>
      </el-form>

      <span slot="footer" class="dialog-footer">
        <el-button size="mini" type="danger" @click="btnGiveTheRes(false)"
          >打回整改</el-button
        >
        <el-button size="mini" type="success" @click="btnGiveTheRes(true)"
          >审核通过</el-button
        >
      </span>
    </el-dialog>
    ...
    <el-dialog
      title="继续下发"
      :visible.sync="keepPostVisible"
      v-if="keepPostVisible"
      width="25%"
    >
      <el-form label-width="80px">
        <el-form-item label="部门选择">
          <treeselect
            @select="eventTreeSelect"
            :options="deptOptions"
            :show-count="true"
            placeholder="请选择归属部门"
          />
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" type="danger" @click="btnConfirmeKeepPost(true)"
          >确定下发</el-button
        >
        <el-button
          size="mini"
          type="success"
          @click="btnConfirmeKeepPost(false)"
          >取消</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    btnGiveTheRes(bolleanVal) {
      ...
      if (bolleanVal) {
        this.$confirm("这个问题将会验收，确定验收吗？", "提示", {
          ...
          .then(async () => {
            const res = await getQuesCheck(this.checkParams);
            console.log(res);
            this.$message({
              type: "success",
              message: "验收成功成功!",
    ...
    handleRemove(file, fileList) {
      this.tempPicList = this.tempPicList.filter(
    ...
    async btnCheckProcess(quesId) {
      ...
      this.processTable = res.data.map((item) => {
        return {
          ...item,
          receiveTimeF: parseTime(
            item.receiveTime,
            "{yy}-{mm}-{dd} {hh}:{ii}:{ss}"
          ),
          processingTimeF: parseTime(
            item.processingTime,
            "{yy}-{mm}-{dd} {hh}:{ii}:{ss}"
          ),
        };
